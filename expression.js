 ...
 * https: //gitee.com/phynos/js-rpn-expression/blob/master/README.md
 * Date: 2018-04-13
 */
...
function isAlphaOrLine(c) {
  return /[a-zA-Z_\$]/.test(c);
};

function isAlphaOrLineOrNumber(c) {
...
function comparePriority(opa, opb) {
  let a = getPriority(opa);
  let b = getPriority(opb);
  if (a > b)
    return -1;
  else if (a == b)
    return 0;
...
CalContext.prototype.calc = function(expression, fixed) {
  //
  this.charIndex = 0;
  //表达式转换
  let finalTokens = this._expr(expression);
  this.finalTokens = finalTokens;
  //执行
  let result = this._execute(finalTokens);
  if (result.isNaN) {
...
CalContext.prototype._next = function(expression) {
  ...
  while (this.charIndex < expression.length) {
    ...
    if (isDigit(c)) {
      let num = parseInt(c);
      let num2 = 0; //如果是浮点数，先用此数记录小数后的部分
      let isFloat = false;
      this.charIndex++;
      while (this.charIndex < expression.length) {
        c = expression[this.charIndex];
...
CalContext.prototype._expr = function(expression) {
  let qcount = 0; //正括号的个数
  let operandStack = []; //操作数堆栈
  let addOperand = function(operand) {
    operandStack.push({
      type: operand.type,
      value: operand.value,
  ...
  let operatorStack = []; //操作符堆栈
  let addOperator = function(operator) {
    operatorStack.push({
      type: operator.type,
  ...
  while ((curToken = this._next(expression)).type != TOKEN_EOF) {
    if (curToken.type == TOKEN_NUMBER) {
      ...
      //循环处理属性调用
      while (true) {
        let cIndex = this.charIndex; // 记录当前字符位置	
        let nextToken = this._next(expression);
        if (nextToken.type == TOKEN_IDENTIFIER) {
          ...
          while (true) {
            //参数表达式						
            let _fpTokens = this._expr(expression);
...
CalContext.prototype._execute = function(exprTokens) {
  ...
  for (let i = 0; i < exprTokens.length; i++) {
    ...
    if (token.type == TOKEN_NUMBER) {
      ...
      let result = this._getCallExpressionResult(token);
      opds.push(result);
      ...
      switch (token.value) {
        case "+":
          opa = opds.pop();
          opb = opds.pop();
          opds.push(new BigNumber(opa).plus(new BigNumber(opb)));
          break;
        case "-":
          opa = opds.pop();
          opb = opds.pop();
          opds.push(new BigNumber(opb).minus(new BigNumber(opa)));
          break;
        case "*":
          opa = opds.pop();
          opb = opds.pop();
          opds.push(new BigNumber(opa).multipliedBy(new BigNumber(opb)));
          break;
        case "/":
          opa = opds.pop();
          opb = opds.pop();
          opds.push(new BigNumber(opb).div(new BigNumber(opa)));
          break;
        case "%":
          opa = opds.pop();
          opb = opds.pop();
          opds.push(new BigNumber(opb).modulo(new BigNumber(opa)));
          break;
        default:
...
CalContext.prototype._getCallExpressionResult = function(callToken) {
  let tail = null,
    obj = null;
  for (let i = 0; i < callToken.callee.length; i++) {
    let name = callToken.callee[i];
    if (i == 0) {
  ...
  if (typeof tail == "function") {
    let args = [];
    //计算参数
    for (let i = 0; i < callToken.args.length; i++) {
      let arg = this._execute(callToken.args[i]);
      args.push(arg);
    }
    //执行
    if (obj.data === tail) { //函数
      //执行 函数
      let result = tail.apply(obj.context || {}, args);
      return result;
    } else { //对象方法
      //执行 对象方法
      let result = tail.apply(obj.data, args);
      return result;
    }
  } else {
